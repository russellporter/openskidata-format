...
describe('getEstimatedRunDifficulty', () => {
  ...
  it('should return correct difficulty for downhill slopes', () => {
    ...
    expect(getEstimatedRunDifficulty(0.1, downhillScale)).toBe(
      RunDifficulty.EASY,
    ) // 10%
    expect(getEstimatedRunDifficulty(0.3, downhillScale)).toBe(
      RunDifficulty.INTERMEDIATE,
    ) // 30%
    expect(getEstimatedRunDifficulty(0.6, downhillScale)).toBe(
      RunDifficulty.ADVANCED,
    ) // 60%
    expect(getEstimatedRunDifficulty(0.9, downhillScale)).toBe(
      RunDifficulty.EXPERT,
    ) // 90%
    expect(getEstimatedRunDifficulty(1.0, downhillScale)).toBe(
  ...
  it('should return correct difficulty for nordic slopes', () => {
    ...
    expect(getEstimatedRunDifficulty(0.05, nordicScale)).toBe(
      RunDifficulty.EASY,
    ) // 5%
    expect(getEstimatedRunDifficulty(0.12, nordicScale)).toBe(
      RunDifficulty.INTERMEDIATE,
    ) // 12%
    expect(getEstimatedRunDifficulty(0.25, nordicScale)).toBe(
      RunDifficulty.ADVANCED,
    ) // 25%
  })
