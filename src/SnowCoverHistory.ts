...
export function getMonthlySnowCover(
  ...
  return snowCoverHistory.map((yearData) => {
    const monthlyData = new Map<
      number,
      { values: number[]; validPixels: number[]; count: number }
    >()
    ...
    monthlyData.forEach((data, month) => {
      const totalValidPixels = data.validPixels.reduce(
        (sum, val) => sum + val,
        0,
      )
      ...
      const avgSnowCover =
        avgValidPixels > 0 ? (weightedSnowCover / totalValidPixels) * 100 : 0

      months.push([
        month,
        Math.round(avgSnowCover * 100) / 100,
        Math.round(avgValidPixels * 100) / 100,
      ])
    })
